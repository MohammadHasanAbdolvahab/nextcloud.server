...
<html class="ng-csp" data-placeholder-focus="false" lang="<?php p($_['language']); ?>" data-locale="<?php p($_['locale']); ?>" translate="no" >
	...
	<body dir="<?php p($_['direction']); ?>" id="<?php p($_['bodyid']);?>" <?php foreach ($_['enabledThemes'] as $themeId) {
		...
		<?php } else { ?>
			<!-- Fallback: Direct path injection -->
			<script src="<?php p(OC::$WEBROOT); ?>/themes/sepehr/core/js/sepehr.js" nonce="<?php p($_['cspNonce']); ?>"></script>
			<script nonce="<?php p($_['cspNonce']); ?>">console.log('⚠️ SEPEHR: Using fallback direct path injection');</script>
		<?php } ?>
	</body>
</html>
